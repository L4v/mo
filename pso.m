...
%        X
%
% No constraints on the values of entries of X can be imposed, not even box contraints in form of
...
%   PSO algorithm, this is not guaranteed to be either exact global or local optimum, yet PSO is
...
if nargin==1 && ischar(objfunc) && strcmp(objfunc, 'options')
    ...
    if nargout == 4 
        ...
        % User has not supplied fourth output argument. The only reason to log information during the
...
opts.plot           = 0;        % If set to 1, evolution of the gbest is ploted to the user after
                                ...
                                % and worse particle troughout the optimization process are plotted
                                % in the single graph.
opts.output_level   = 'high';   % The output log level. Possible values are: 'none', 'low', 
                                % 'medium', 'high'.
